    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < numberOfMessages; i++)
            {
                ...
                string pattern = @"@(?<planet>[a-zA-z]+).*:[-]?(?<population>[0-9]+).*!(?<attackType>A?|D?)!.*->[-]?(?<soldierCount>[0-9]+)";
                ...
                if (match.Success)
                {
                    if (match.Groups["planet"].Index < match.Groups["population"].Index &&
                    match.Groups["attackType"].Index < match.Groups["soldierCount"].Index &&
                    match.Groups["population"].Index < match.Groups["attackType"].Index)
                    {
